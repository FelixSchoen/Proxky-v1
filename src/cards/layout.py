...
from src.utility.util_xml import ind_shift_coordinates, change_object_transparency, \
    change_text_color, change_text_style
from src.utility.util_misc import split_string_along_regex, mm_to_pt
from src.utility.variables import ids, VALUE_MODAL_HEIGHT, regex_template_oracle, \
    VALUE_SHIFT_WITHOUT_ORACLE_WITHOUT_VALUE, \
...
def card_layout_double_faced(id_sets):
    for i, id_set in enumerate(id_sets):
        ...
        ind_shift_coordinates(oracle_text, y_coordinates=[shift_by, shift_by, 0, 0])
        ...
        for box_id in planeswalker_boxes:
            ...
            ind_shift_coordinates(box, y_coordinates=[shift_by, shift_by, shift_by, shift_by])
...
def card_layout_no_value(id_set):
    ...
    if ids.MASK_COLOR_INDICATOR_BOT_O in id_set:
        ...
        ind_shift_coordinates(color_indicator_bot, y_coordinates=[SHIFT_BY, SHIFT_BY, SHIFT_BY, SHIFT_BY])
...
def card_layout_token(id_set, card):
    oracle_entries = split_string_along_regex(card.oracle_text, *regex_template_oracle)
...
def card_layout_no_oracle_text(id_set, card):
    ...
    ind_shift_coordinates(backdrop,
...
def card_layout_transparent_body_art(id_set):
    ...
    artwork = tree.find(".//Rectangle[@Self='" + id_set[ids.ARTWORK_O] + "']")
    ind_shift_coordinates(artwork, y_coordinates=[0, 0, VALUE_SHIFT_ARTWORK_FULL_BODY,
    ...
    for object_id in rectangles_50:
        ...
        change_object_transparency(element, 50)

    for object_id in rectangles_85:
        element = tree.find(".//Rectangle[@Self='" + object_id + "']")
        change_object_transparency(element, 85)

    for object_id in groups_75:
        element = tree.find(".//Group[@Self='" + object_id + "']")
        change_object_transparency(element, 75, mode="")
...
def card_layout_full_body_art(id_set):
    ...
    backdrop = tree.find(".//Rectangle[@Self='" + id_set[ids.BACKDROP_O] + "']")
    change_object_transparency(backdrop, 85)
    ind_shift_coordinates(backdrop,
    ...
    change_text_color(artist, "Color/Paper")
    change_text_style(artist, "Display Bold")
    ...
    change_text_color(collector_information, "Color/Paper")
    change_text_style(collector_information, "Display Bold")
    ...
    change_text_color(value, "Color/Paper")
